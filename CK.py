...
class CK(data.Dataset):
    ...
    def __init__(self, split='Training', fold = 1, transform=None):
        ...
        ############revised by YH.
        '''with h5py.File('./data/CK_data.h5', 'r', driver='core') as file:
            self.data = file'''
        with h5py.File('./data/CK_data.h5', 'r', driver='core') as file:
            # Access and load the data you need from the file
            self.data = file[:]  # Replace 'your_dataset_name' with the actual dataset name
        ############################
        #add by YH.
        '''a=self.data.keys()
        dataset=self.data['CK']'''
        ###########################
